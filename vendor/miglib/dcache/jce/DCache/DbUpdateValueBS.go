...
type DbUpdateValueBS struct {
	Op    Op
	Value []int8
	Type  DataType
}

func (st *DbUpdateValueBS) resetDefault() {
}

func (st *DbUpdateValueBS) ReadFrom(_is *codec.Reader) error {
	var err error
	var length int32
	var have bool
	var ty byte
...
func (st *DbUpdateValueBS) ReadBlock(_is *codec.Reader, tag byte, require bool) error {
	var err error
	var have bool
	st.resetDefault()
	...
	if !have {
		if require {
			return fmt.Errorf("require DbUpdateValueBS, but not exist. tag %d", tag)
		} else {
			return nil
		}
	}
...
func (st *DbUpdateValueBS) WriteTo(_os *codec.Buffer) error {
	var err error
...
func (st *DbUpdateValueBS) WriteBlock(_os *codec.Buffer, tag byte) error {
	var err error
