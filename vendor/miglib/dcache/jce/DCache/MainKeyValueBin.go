...
type MainKeyValueBin struct {
	MainKey string
	Value   []int8
	Ret     int32
}

func (st *MainKeyValueBin) resetDefault() {
}

func (st *MainKeyValueBin) ReadFrom(_is *codec.Reader) error {
	var err error
	var length int32
	var have bool
	var ty byte
...
func (st *MainKeyValueBin) ReadBlock(_is *codec.Reader, tag byte, require bool) error {
	var err error
	var have bool
	st.resetDefault()
	...
	if !have {
		if require {
			return fmt.Errorf("require MainKeyValueBin, but not exist. tag %d", tag)
		} else {
			return nil
		}
	}
...
func (st *MainKeyValueBin) WriteTo(_os *codec.Buffer) error {
	var err error
...
func (st *MainKeyValueBin) WriteBlock(_os *codec.Buffer, tag byte) error {
	var err error
