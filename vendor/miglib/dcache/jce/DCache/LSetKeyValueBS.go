...
func (st *LSetKeyValueBS) resetDefault() {
	st.Version = 0
	st.Dirty = true
	st.ExpireTimeSecond = 0
}

func (st *LSetKeyValueBS) ReadFrom(_is *codec.Reader) error {
	var err error
	var length int32
	var have bool
	var ty byte
...
func (st *LSetKeyValueBS) ReadBlock(_is *codec.Reader, tag byte, require bool) error {
	var err error
	var have bool
	st.resetDefault()
	...
	if !have {
		if require {
			return fmt.Errorf("require LSetKeyValueBS, but not exist. tag %d", tag)
		} else {
			return nil
		}
	}
...
func (st *LSetKeyValueBS) WriteTo(_os *codec.Buffer) error {
	var err error
...
func (st *LSetKeyValueBS) WriteBlock(_os *codec.Buffer, tag byte) error {
	var err error
