...
type SKeyResultBS struct {
	KeyItem []int8
	Ret     int32
}

func (st *SKeyResultBS) resetDefault() {
}

func (st *SKeyResultBS) ReadFrom(_is *codec.Reader) error {
	var err error
	var length int32
	var have bool
	var ty byte
...
func (st *SKeyResultBS) ReadBlock(_is *codec.Reader, tag byte, require bool) error {
	var err error
	var have bool
	st.resetDefault()
	...
	if !have {
		if require {
			return fmt.Errorf("require SKeyResultBS, but not exist. tag %d", tag)
		} else {
			return nil
		}
	}
...
func (st *SKeyResultBS) WriteTo(_os *codec.Buffer) error {
	var err error
...
func (st *SKeyResultBS) WriteBlock(_os *codec.Buffer, tag byte) error {
	var err error
