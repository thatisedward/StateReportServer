...
func (st *SKeyValueBS) resetDefault() {
}

func (st *SKeyValueBS) ReadFrom(_is *codec.Reader) error {
	var err error
	var length int32
	var have bool
	var ty byte
	...
	if ty == codec.LIST {
		...
		for i1, e1 := int32(0), length; i1 < e1; i1++ {
			...
			err = _is.Read_int8(&st.Value[i1], 0, false)
			if err != nil {
				return err
			}
		}
...
func (st *SKeyValueBS) ReadBlock(_is *codec.Reader, tag byte, require bool) error {
	var err error
	var have bool
	st.resetDefault()
	...
	if !have {
		if require {
			return fmt.Errorf("require SKeyValueBS, but not exist. tag %d", tag)
		} else {
			return nil
		}
	}
...
func (st *SKeyValueBS) WriteTo(_os *codec.Buffer) error {
	var err error
...
func (st *SKeyValueBS) WriteBlock(_os *codec.Buffer, tag byte) error {
	var err error
