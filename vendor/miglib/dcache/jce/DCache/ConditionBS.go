...
type ConditionBS struct {
	FieldName string
	Op        Op
	Value     []int8
}

func (st *ConditionBS) resetDefault() {
}

func (st *ConditionBS) ReadFrom(_is *codec.Reader) error {
	var err error
	var length int32
	var have bool
	var ty byte
...
func (st *ConditionBS) ReadBlock(_is *codec.Reader, tag byte, require bool) error {
	var err error
	var have bool
	st.resetDefault()
	...
	if !have {
		if require {
			return fmt.Errorf("require ConditionBS, but not exist. tag %d", tag)
		} else {
			return nil
		}
	}
...
func (st *ConditionBS) WriteTo(_os *codec.Buffer) error {
	var err error
...
func (st *ConditionBS) WriteBlock(_os *codec.Buffer, tag byte) error {
	var err error
